...
当前阶段优先验证多实例 gateway 下的设备路由、断点续传、固件本地缓存重新加载和分包链路，推荐实例规划为：
...
mock-device 作为联调和压测工具，不属于真实生产核心链路。当前只保留 1 个 mock-device 实例，由它在设备每次建立 TCP 连接时随机选择 gateway endpoint：
...
这样更接近真实设备通过负载均衡随机接入不同 gateway 的效果，也能验证设备断线后重连到另一台 gateway 时，新 gateway 本地 JVM 固件缓存不存在、需要重新从 MinIO 加载固件的场景。
...
其中 mock-device 只用于演示、联调或压测。真实生产设备接入时，可以不部署 mock-device。
